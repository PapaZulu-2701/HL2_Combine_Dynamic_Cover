...
CNPC_MetroPolice::CNPC_MetroPolice()
{
	...
	m_hLowCoverProp = NULL;				// [MODIFICATION] -  Dynamic Cover
	m_hHighCoverProp = NULL;			// [MODIFICATION] -  Dynamic Cover
	m_hLastArrivedProp = NULL;			// [MODIFICATION] -  Dynamic Cover
	m_flNextPropSearchTime = 0;			// [MODIFICATION] -  Dynamic Cover
...
// ============================================
int CNPC_MetroPolice::CountNPCsUsingProp(CBaseEntity* pProp)
{
	if (!pProp)
		return 0;
	...
	{
		// Tentar converter para Combine
		CNPC_MetroPolice* pMetroPolice = dynamic_cast<CNPC_MetroPolice*>(pEntity);
		if (!pMetroPolice || pMetroPolice == this) // Não contar a si mesmo
			continue;
		...
		if (pMetroPolice->GetHealth() <= 0 || !pMetroPolice->IsAlive())
			continue;

		// Check if using the same prop
		if (pMetroPolice->m_hLowCoverProp == pProp || pMetroPolice->m_hHighCoverProp == pProp)
		{
			count++;
		}
	}
...
bool CNPC_MetroPolice::DynamicPropCover()
{
...
int CNPC_MetroPolice::SelectCombatSchedule()
{
	...
	{
		bool hasLow = HasCondition(COND_METROPOLICE_HAS_LOW_COVER);
		bool hasHigh = HasCondition(COND_METROPOLICE_HAS_HIGH_COVER);
		...
		if (hasLow && hasHigh)
		{
			...
			if (lowValid && highValid)
			{
				...
				return useLowCover ? SCHED_METROPOLICE_MOVE_TO_LOW_COVER : SCHED_METROPOLICE_MOVE_TO_HIGH_COVER;
			}
			else if (lowValid)
			{
				Msg("[DEBUG] SelectSchedule: Only LOW COVER in valid range\n");
				return SCHED_METROPOLICE_MOVE_TO_LOW_COVER;
			}
			else if (highValid)
			{
				Msg("[DEBUG] SelectSchedule: Only HIGH COVER in valid range\n");
				return SCHED_METROPOLICE_MOVE_TO_HIGH_COVER;
			}
			else
			{
...
void CNPC_MetroPolice::StartTask( const Task_t *pTask )
{
	switch (pTask->iTask)
	{
	case TASK_METROPOLICE_MOVE_TO_HIGH_COVER:  // [MODIFICATION-Dynamic Cover]
	{
		...
		if (heightDifference > 100.0f)
		{
			...
			ClearCondition(COND_METROPOLICE_HAS_HIGH_COVER);
	...
	case TASK_METROPOLICE_MOVE_TO_LOW_COVER: // [MODIFICATION-Dynamic Cover]
	{
		...
		if (distToTarget <= 50.0f)
		{
			ClearCondition(COND_METROPOLICE_HAS_LOW_COVER);
	...
	case TASK_METROPOLICE_MEMORIZE_GRENADE: // [MODIFICATION] An instantaneous task that identifies and 'memorizes' the grenade from a danger sound.
	{
		...
		else
		{
			...
			TaskComplete();	// Fails the entire schedule because no target could be found.
		}
	...
	case TASK_METROPOLICE_PLAY_KICK_ANIMATION: // [MODIFICATION]
	{
		...
		if (kickSequence != ACTIVITY_NOT_AVAILABLE)
		{
			...
			m_flPlaybackRate = 2.0f; //This seems the best speed to look more "natural"
	...
	if (GetState() == NPC_STATE_COMBAT && GetEnemy())
	{
		...
		else
		{
			...
			if (m_hLowCoverProp)
			{
				...
				{
					SetCondition(COND_METROPOLICE_HAS_LOW_COVER);
			...
			if (m_hHighCoverProp)
			{
				...
				if (GetNavigator()->CanFitAtPosition(testPos, MASK_NPCSOLID))
				{
					SetCondition(COND_METROPOLICE_HAS_HIGH_COVER);
...
AI_BEGIN_CUSTOM_NPC( npc_metropolice, CNPC_MetroPolice )
	...
	DECLARE_TASK(TASK_METROPOLICE_MOVE_TO_LOW_COVER) // [MODIFICATION] -  Dynamic Cover - START
	DECLARE_TASK(TASK_METROPOLICE_MOVE_TO_HIGH_COVER)// [MODIFICATION] -  Dynamic Cover - START
...
//	DECLARE_CONDITION( COND_METROPOLICE_START_POLICING );
	...
	DECLARE_CONDITION(COND_METROPOLICE_HAS_HIGH_COVER) // [MODIFICATION] -  Dynamic Cover - START
	DECLARE_CONDITION(COND_METROPOLICE_HAS_LOW_COVER) // [MODIFICATION] -  Dynamic Cover - START
...
DEFINE_SCHEDULE 
(
	SCHED_METROPOLICE_MOVE_TO_HIGH_COVER,

	"	Tasks"
	"		TASK_STOP_MOVING						0"
	"		TASK_METROPOLICE_MOVE_TO_HIGH_COVER		0"
...
DEFINE_SCHEDULE  
(
	SCHED_METROPOLICE_MOVE_TO_LOW_COVER,

	"   Tasks"
	"		TASK_STOP_MOVING					    0"
	"		TASK_METROPOLICE_MOVE_TO_LOW_COVER		0"
	"		TASK_RUN_PATH_TIMED						4"
	"		TASK_WAIT_FOR_MOVEMENT					0"
	"		TASK_FACE_ENEMY							0"
	"		TASK_RANGE_ATTACK1						0"
